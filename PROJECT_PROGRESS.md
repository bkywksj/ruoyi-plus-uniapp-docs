...
> **最后更新**: 2026-09-16（技能体系数据全站对齐为 61 技能/19 命令/3 钩子：claude-code.md 补齐 dev-startup 与 collaborating-with-antigravity 两个缺失技能小节，61 个小节与源码一一对应；首页、钩子文档、实践索引同步修正；对比页第十四章精简为差异视角，明细交由 practices 承载）
> **整体完成度**: 100% (505/505)
> **总行数**: 767,600+ 行
>
...
| 任务调度 (SnailJob) | `backend/modules/business/job.md` | ✅ |
| 定时任务 (ruoyi-job) | `backend/modules/job.md` | ✅ |
...
| 认证页 (AuthLayout) | `frontend/layout/auth-layout.md` | ✅ |
...
| 等保二级合规 | `practices/features/security-compliance.md` | ✅ |
...
| 文档 | 路径 | 状态 |
|------|------|------|
| Claude Code 深度集成 | `practices/ai/claude-code.md` | ✅ |
...
| 日期 | 里程碑 |
|------|--------|
| 2026-09-16 | 🔐 BOM 补录 fastjson 1.2.84 安全顶版章节（CVE-2026-16723），并修正 13 处版本表与 pom 不一致（LangChain4j 0.35.0→1.14.1、Warm-Flow 1.8.1→1.8.9 等） |
| 2026-09-16 | 🧹 全站清理 1,375 处 `参考: src/xxx:行号` 引用格式（47 个文件），对齐 CLAUDE.md 第 7 条规范 |
| 2026-09-16 | 📱 移动端同步：wd-paging 竞态与 pageSize 优先级说明，登录页补图形验证码前置 |
| 2026-09-16 | 🎨 前端同步：新增认证页文档（502 行），补双列布局模式、页签溢出交互与视觉层次，修正 MenuLayoutMode 枚举缺失 DualColumn |
| 2026-09-16 | ⏰ 新增定时任务模块文档（1,039 行），补齐 ruoyi-job 的文档空白 |
| 2026-09-16 | 🔄 依赖版本同步：17 个文件 44 处版本号对齐源码 pom（Spring Boot 3.5.16 / Sa-Token 1.45.0 / Hutool 5.8.43 / SnailJob 1.10.0 等） |
| 2026-09-16 | 💳 支付文档补齐支付宝当面付（扫码+付款码）与条码支付撤销兜底，修正已废弃的微信V3公钥回退描述 |
| 2026-09-16 | 🔐 认证文档补齐发码三级限流与图形验证码前置，修正验证码位数(6→4)与有效期(5→2分钟)错误 |
| 2026-09-16 | ☕ Java 17→21 全站同步（20 个文件 53 处），修正「JDK17 不支持虚拟线程」等语义冲突表述 |
| 2026-09-16 | 🔒 新增等保二级合规文档（699 行），覆盖身份鉴别 8.1.4.1 a、安全审计 8.1.4.3 c、数据备份恢复 8.1.4.7 a |
| 2026-09-15 | 🔧 架构描述纠偏：清理 14 处已废弃的 `IBaseService`/`BaseServiceImpl` 继承示例，统一为「Service 不继承基类 + 注入 DAO」现行四层架构；同步修正首页及对比表中无基线的百分比文案 |
| 2026-02-02 | 📝 重构字典管理模块文档,从367行扩展到1446行 |
| 2026-01-22 | ✅ 新增 Claude Code 深度集成文档 |
