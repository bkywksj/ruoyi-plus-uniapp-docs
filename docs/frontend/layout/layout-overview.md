...
- 左侧窄列只显示一级模块的图标，右侧列显示当前模块的子菜单
- 本质仍是侧边栏形态（`topNav = false`、`sidebar.hide = false`），不占用顶部空间
- 适合一级模块较多、但每个模块子菜单不深的系统

**两种宽度状态**：

| 状态 | 侧栏总宽 | 触发条件 |
|------|---------|---------|
| 完整双列 | `284px`（74 图标列 + 210 子菜单列） | 常规情况 |
| 仅图标列 | `74px` | 侧栏折叠态 / 当前在首页 / 当前模块是单页面模块 |

**一级模块的判定规则**抽在 `layouts/components/Sidebar/dualModule.ts`，因为有两个使用方必须得出同一结论：

- `DualSidebar.vue` —— 决定右侧子菜单列渲不渲染
- `Layout.vue` —— 决定外层 `.sidebar-container` 的宽度

两边各写一份的话，一旦只改了其中一处，就会出现「子菜单列已隐藏、外层仍按 284px 留位」的空白黑块。因此两者都调用同一个 `shouldRailOnly()`。

**单页面模块的特殊处理**：一级普通菜单（`sys_menu.menu_type='C'` 且 `parent_id=0`，如「我的应用」）在后端会被包成 Layout 父路由，父路由没有 `meta` 也没有 `name`，真正的标题与图标在唯一的子路由上。

这带来两个后果，`dualModule.ts` 都做了处理：

1. 直接读父路由的 `meta` 会得到空标题空图标，图标列上就是一排「有高度、没内容」的空白块——所以用 `displayRoute()` 取那个唯一子路由的展示信息顶上，只影响展示，跳转仍按模块本身算
2. 这类模块本身就是终点页，右列再挂一条同名菜单是多余的一层——所以 `shouldRailOnly()` 把它归入「只保留图标列」

**路由排除**：`/redirect`、`/login`、`/register`、`/401`、`/user`、`/index`、`/` 这些系统与兜底路由不进图标列。首页作为固定置顶入口单独处理，不参与模块列表。

由当前路由反推所属模块时，非模块页返回空串而**不做兜底高亮**——否则会误点亮第一个模块。
