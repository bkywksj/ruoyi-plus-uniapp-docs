...
- **溢出可发现性** - 左右步进箭头、两侧渐变遮罩，页签开多时不会被"悄悄裁掉"
- **页签总览** - 带数量角标的下拉列出全部页签，可滚动、当前页高亮、支持单项关闭
- **右键目标标识** - 虚线描边标出右键菜单的作用对象，与当前页的实线描边区分
...
页签开多之后右侧内容会被直接裁掉。`ScrollPane` 其实一直支持滚轮横向滚动，但滚动条被 `scrollbar-width` 与 `::-webkit-scrollbar` 三重隐藏，右侧又没有任何箭头或遮罩，**可发现性为零**——用户只会以为后面的页签丢了。

现在右侧有一块固定操作区：左右步进箭头、带数量角标的页签总览下拉、全局操作下拉。

#### 箭头常驻占位，只切换禁用态

箭头**不做「不溢出即隐藏」**。隐藏会改变右侧区宽度，进而改变滚动区宽度并再次触发 `ResizeObserver`；宽度处于临界值时会无限震荡。因此箭头始终占位，仅在不可滚动时置灰。

#### 1px 滚动容差

`canScrollLeft` / `canScrollRight` 的判定带 1px 容差：

```typescript
const canScrollRight = computed(() =>
  scrollWidth.value - clientWidth.value - scrollLeft.value > 1
)
```

浏览器缩放比非 100% 时，`scrollWidth` 与 `clientWidth` 存在亚像素误差。不留容差会导致已滚到底仍判定"可滚动"，箭头永远点不灭。

#### 单次滚动 80% 可视宽度

`scrollByStep()` 每次滚动 80% 而非 100%，**刻意留一段重叠区**——用户能看到上一屏的最后一两个页签，保持视觉参照，不会滚一下就完全失去方位。

#### ResizeObserver 同时观察 wrap 与 view

侧边栏折叠、窗口缩放、增删页签都会改变可滚动状态，两个元素都要观察才能覆盖全部场景。

#### 两个下拉按钮用原生 title 而非 el-tooltip

`el-dropdown` 自身即基于 popper，再套一层 `el-tooltip` 作 trigger 会**争抢事件导致下拉点不开**。

#### 渐变遮罩

溢出时两侧显示渐变遮罩提示内容被裁切，`pointer-events: none` 保证不挡点击。

#### 移动端降级

`≤768px` 隐藏步进箭头，仅保留总览与全局操作下拉——小屏上箭头占的宽度比它提供的价值更贵。

### 视觉层次设计

页签的三种状态用**不同维度**的信号区分，避免互相干扰：

| 状态 | 背景 | 描边 | 文字 |
|------|------|------|------|
| 普通 | `transparent` | `--el-border-color-lighter` | 常规色 |
| 激活 | 主题色浅底 | 主题色实线 | 主题色 |
| 右键目标 | 不设背景 | 主题色**虚线** | 主题色 |

几个刻意的取舍：

- **非激活页签背景改 `transparent`**。原先逐个填 `--el-fill-color-blank`（白），而栏底是 `#fafbfc`，几乎同色，边框反倒成了整条栏唯一的视觉信息，整排像一堵"小方框墙"。改透明后整排从方框墙变成轻轮廓。
- **激活页签用三重信号**（浅底 + 描边 + 文字色）。原先只靠文字变色加 1px 上浮，开到八九个就找不到当前页。
- **右键目标刻意不设背景色、不加字重**——那两个是 active 的专属信号，占用会让两种状态糊在一起。两者重合时叠加为「浅底 + 虚线」，仍可分辨。
- **虚线只换 `border-style` 不改宽度**，避免标签宽度变化触发滚动区的 `ResizeObserver` 重算。

右键目标标识解决的是一个真实问题：菜单弹出后鼠标移到菜单上，hover 随即消失；被右键的页签若不是当前页，就失去了全部视觉标识，而「关闭当前 / 关闭左侧 / 关闭右侧」全以它为基准。

#### 空白处右键

页签栏空白处右键也会弹出菜单，作用于当前页签。页签自身的 `contextmenu` 加了 `.stop` 防止重复触发。
