...
本项目在 `.claude/commands/` 目录下定义了 **18 个自定义命令**，分为 6 大类。

> 说明：源码项目额外包含若干 local 上游同步命令（如 `sync-local`、`sync-branches-local`），仅在源码开发环境使用，不列入通用文档。
...
| 命令 | 用途 | 使用时机 |
|------|------|---------|
| `/kickoff` | 一句话起新项目的前门编排器 | 从零起一个新业务项目：联网调研出需求 → 工作站出原型 → `project-init` 孵化 → 起飞 `/dev-loop`，前段每步带人工门把关「做什么」，后段交给自主循环做「怎么做」 |
| `/dev-loop` | 自主循环开发的「一轮标准作业」 | 配合内置 `/loop` 用 `/loop /dev-loop` 自主连续开发，直到 `docs/tasks/active/` 任务台账全部勾完。本命令是编排器：CRUD 走 `/dev`、原型转码走 `html-to-code`、UI 截图验收复用 e2e-test、汇总用 `/update-status`，不重复造轮子 |
| `/loop-gen` | 现生成一条量身定做的 `/loop` 提示词 | 当本次意图不是「按需求建功能」（如还原原型/批量修 bug/补测试/重构/迁移/清理）时，听一句意图，产出填好真相源/每轮动作/验证门/停止条件的 `/loop` 提示词供直接粘贴起飞 |

### 部署与同步命令（4 个）

| 命令 | 用途 | 使用时机 |
|------|------|---------|
| `/deploy` | 一键自动化部署 | 项目构建后部署到生产服务器（JAR / Docker） |
| `/framework-sync` | 框架同步 | 将 ruoyi-plus-uniapp 框架原仓库的更新同步到基于模板创建的子项目 |
| `/sync-delivery` | 交付副本同步 | 把主项目单向镜像同步成不带 git 历史、不带技能体系、不带内部协作文档的交付副本；支持首次创建、增量同步、`--dry-run` 预览与多客户预设 |
| `/strip-modules` | 业务模块裁剪 | `/sync-delivery` 的下游：在交付目录里删除指定业务模块（删目录 + 改 pom.xml），内置商城/IoT/支付/AI 等预设，支持 `--list`/`--preview`/`--verify`（裁剪后跑 `mvn compile`） |
