...
|---------|------|------|------|------|------|------|
| 主键字段 | 否 | 否 | 是 | 否 | 是 | 自动生成,不允许手动输入 |
| 普通字段 | 是 | 是 | 是 | 是 | 否 | 常规业务字段 |
| create_time | 否 | 否 | 是 | 否 | 否 | 系统自动填充 |
| update_time | 否 | 否 | 否 | 否 | 否 | 系统自动更新 |
| create_by | 否 | 否 | 否 | 否 | 否 | 系统自动填充 |
| update_by | 否 | 否 | 否 | 否 | 否 | 系统自动填充 |
| is_deleted | 否 | 否 | 否 | 否 | 否 | 逻辑删除标记 |
| tenant_id | 否 | 否 | 否 | 否 | 否 | 租户ID,自动填充 |
| version | 否 | 否 | 否 | 否 | 否 | 乐观锁版本号 |
| remark | 是 | 是 | 否 | 否 | 否 | 备注字段,不在列表显示 |
