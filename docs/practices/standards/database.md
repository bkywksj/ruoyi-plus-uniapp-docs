...
- <DotRed/> **必须(MUST)** - 强制执行,违反将导致代码审查不通过
- <DotYellow/> **应该(SHOULD)** - 强烈建议,特殊情况需说明原因
- <DotGreen/> **建议(MAY)** - 推荐做法,可根据实际情况调整
...
- <Ok/> 根据实际业务需求选择长度
- <Ok/> 预留适当扩展空间
- <No/> 禁止所有字段都用 VARCHAR(255)
- <No/> 禁止 VARCHAR 超过 3000
...
- <No/> 不需要 `update_by`, `update_time` (日志不修改)
- <Ok/> 可以冗余字段避免关联查询
- <Ok/> 必须有时间索引
- <Ok/> 考虑分区或定期归档
...
- <DotRed/> 必须遵守的规范是底线,不可违反
- <DotYellow/> 应该遵守的规范是最佳实践,特殊情况需说明原因
- <DotGreen/> 建议遵守的规范是推荐做法,可根据实际调整
