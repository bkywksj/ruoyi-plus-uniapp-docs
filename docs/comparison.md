...
技能不是孤立的工具箱。按交付顺序串起来，它们构成一条从「一句话需求」到「线上服务」的流水线：需求孵化产出需求文档与建表施工图，原型设计产出高保真 HTML，建表编码进入自主循环，再经 E2E 验收、部署上线、官网与文档站建设、交付客户，最后把踩过的坑沉淀回技能库，供下一个项目复用。

流水线上有两道**人工决策门**，卡在全流程最便宜的两个返工点上：

| 关卡 | 拦截内容 | 为什么卡在这里 |
|------|---------|--------------|
| **人工门 ①** | 需求文档未确认，不进原型 | 此时改需求只改 Markdown，不碰任何代码 |
| **人工门 ②** | 原型视觉未确认，不写一行业务代码 | 此时改视觉只改 HTML，不返工后端与联调 |

两道门之外的环节交给 AI 自转：编码阶段的自主循环会自己编写测试、运行报错、读取日志、修复闭环；敏感 Token 与密钥由桌面凭据保险箱托管，不进 Git、全程不落盘。两个标注为**不可逆**的环节——建立远程私有仓库、部署上线与修改线上配置——则强制先演练再执行。

<div style="max-width: 62%; margin: 28px auto; text-align: center;">
  <ImagePreview src="/images/ai-delivery-pipeline.webp" width="100%" height="auto" object-fit="contain" />
  <p style="margin-top: 10px; color: #888; font-size: 14px;">全流程交付流水线全景图 · 点击查看大图</p>
</div>
