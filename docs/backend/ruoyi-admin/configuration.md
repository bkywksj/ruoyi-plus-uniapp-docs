...
**注意：** 虚拟线程是 JDK 21 起的正式特性。项目已基于 JDK 21 构建，该配置默认开启；若降级到 JDK 17 等更低版本运行，需设置为 `false` 或删除此配置，否则启动会失败。
...
**说明：**
- JDK 21 环境（项目默认）推荐使用虚拟线程，无需开启此线程池
- 低于 JDK 21 的环境无法使用虚拟线程，此时应开启此配置并按负载调整参数
