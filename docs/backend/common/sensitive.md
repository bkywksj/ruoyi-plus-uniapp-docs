...
本模块除 `@Sensitive` 之外，还承载了字段级权限的 JSON 读出口实现（`FieldPermissionSerializerModifier` 与 `FieldPermissionPropertyWriter`）。两套机制定位不同，需要理解它们的叠加关系。

#### 定位差异

| 维度 | `@Sensitive` 注解 | 字段级权限配置 |
|------|------------------|---------------|
| 生效时机 | 编译期固定 | 运行期可调 |
| 粒度 | 全员统一（按角色/权限粗判） | 按角色 / 部门 / 用户逐字段配置 |
| 能表达的档位 | 脱敏 / 不脱敏 | 隐藏 / 脱敏 / 只读 / 可写 |
| 覆盖出口 | JSON 序列化 | JSON 序列化 + Excel 导出 + JSON 写入 + Excel 导入 |
| 配置方式 | 改代码重新发布 | 管理端「系统管理 → 字段权限」 |

简单说：`@Sensitive` 是**默认安全基线**，字段权限是**差异化授权**。

#### 叠加规则

字段权限的属性写出器在写每个属性前查一次当前用户的动态决策，按三种结果分支：

| 动态决策 | 行为 |
|---------|------|
| 无决策（返回 null） | **原样委派**，字段自身的 `@Sensitive` / `@SerialMap` 照常生效 |
| 隐藏 | 整个属性不写出，JSON 里连 key 都没有 |
| 脱敏 | 按配置策略打码；未配策略则回落字段自身 `@Sensitive` 的策略，再没有则用通用掩码 |
| 只读 / 可写 | 明文。若字段带 `@Sensitive`，此处**覆盖**注解直接写原值 |

最后一行是关键：正因为能覆盖注解，「`phone` 字段带 `@Sensitive`，但要让 HR 角色看到原值」这个需求才做得到——不覆盖的话注解会无条件打码，配了也永远看不到明文。

带 `@SerialMap` 的字段是例外，**不做覆盖**：`@SerialMap` 是展示转换（ID→名称）而非安全控制，绕过它会输出原始 ID，属于改坏不是放权。

#### 注解基线

字段权限的配置表只存「与注解基线不同」的差异行，基线规则直接来自本模块的注解：

| 字段情况 | 注解基线档位 |
|---------|-------------|
| 带 `@Sensitive` 且为 String 类型 | 脱敏 |
| 其余全部字段 | 可写 |

基线由应用启动时的扫描器反射算好写进 Redis——热路径禁止反射，只能在启动期一次算完。

非 String 字段上的 `@Sensitive` 实际不生效，算基线时会**跳过**，否则会凭空锁住一个本来明文输出的字段。这类字段在字段权限配置页会标「注解不覆盖」红字提示，意思是它只能靠动态配置兜底。

#### 对现有代码的影响

- 未标 `@FieldResource` 的类**零影响**，也零开销：Jackson 的 `changeProperties` 按类调用一次且结果被永久缓存，未标注解的类之后每次序列化根本不进入拦截逻辑
- 已有的 `@Sensitive` 行为在「无动态决策」时完全不变
- `field-permission.enabled=false` 时拦截器整个不注册，行为完全回到只有 `@Sensitive` 的版本
