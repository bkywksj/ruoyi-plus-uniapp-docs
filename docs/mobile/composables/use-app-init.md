...
|------|-------------|---------|-----------|------|
| APP | 否 跳过 | 否 手动登录 | 是 登录后连接 | 无需租户ID，用户手动登录 |
| 普通H5 | 否 跳过 | 否 手动登录 | 是 登录后连接 | 非公众号H5无需初始化 |
| 微信公众号H5 | 是 执行 | 是 授权登录 | 是 登录后连接 | 检测URL中的code和state |
| 支付宝H5 | 是 执行 | 否 手动登录 | 是 登录后连接 | 仅获取租户ID |
| 微信小程序 | 是 执行 | 是 静默登录 | 是 登录后连接 | wx.login获取code登录 |
| 支付宝小程序 | 是 执行 | 是 静默登录 | 是 登录后连接 | my.getAuthCode登录 |
| 百度小程序 | 是 执行 | 是 静默登录 | 是 登录后连接 | swan.login登录 |
| 抖音小程序 | 是 执行 | 是 静默登录 | 是 登录后连接 | tt.login登录 |
| QQ小程序 | 是 执行 | 是 静默登录 | 是 登录后连接 | qq.login登录 |
