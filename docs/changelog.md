...
- **[<Icon icon="lucide:tv" />新特性全面解析](https://www.bilibili.com/video/BV1YrtMzvEaT/)**

### 分支说明

框架提供 5 个分支变体，业务代码与规范一致，按「租户模型 × 技术栈」区分：

| 分支 | Spring Boot | JDK | 多租户 | 工作流 | 说明 |
|------|:---:|:---:|:---:|:---:|------|
| **master** | 3.5.x | 21 | 是 | 否 | 默认分支，多租户 |
| **single** | 3.5.x | 21 | 否 | 否 | 移除所有多租户逻辑（单租户版） |
| **workflow** | 3.5.x | 21 | 是 | 是 | 多租户 + warm-flow 工作流 |
| **6.x** | 4.1.0 | 21 | 是 | 是 | Spring Boot 4 全功能版（对标 workflow） |
| **6.x-single** | 4.1.0 | 21 | 否 | 是 | Spring Boot 4 单租户 + 工作流版 |
...
#### 微信支付 v2/v3 智能切换
- **自动版本识别** <Icon icon="lucide:star" />：根据配置自动选择 v2 或 v3 API，无需手动指定
...
#### 证书配置灵活性增强
- **多种配置方式**：
  - <Ok/> 文件路径模式：支持 `classpath:` 和文件系统路径
  - <Ok/> PEM 内容模式：直接配置证书内容，无需文件
  - <Ok/> 环境分离：开发/生产证书自动切换
...
本次更新重点关注**物联网通信**、**AI能力集成**、**页面设计器**、**多媒体处理**、**客户端工具包**和**智能开发体验**,为框架注入了强大的企业级功能:

- <Icon icon="lucide:rocket" /> **物联网就绪**：MQTT + RocketMQ 提供完整的物联网解决方案
- <Icon icon="lucide:bot" /> **AI赋能**：LangChain4j 集成主流大模型,开启智能化应用时代
- <Icon icon="lucide:image" /> **页面设计器**：可视化拖拽设计、30+组件、AI智能生成、一键代码导出
- <Icon icon="lucide:palette" /> **多媒体能力**：海报生成、TTS 语音合成等多媒体处理能力
- <Icon icon="lucide:hammer" /> **工具齐全**：HTTP客户端、加密工具、ID生成器等开箱即用
- <Icon icon="lucide:brain" /> **智能开发**：Claude Code 上下文工程,AI辅助开发体验升级
- <Icon icon="lucide:library" /> **文档完善**：在线文档站点,每个模块详细文档和示例

---

## 七、2026年重大更新 (1月-7月)

### 7.1 Spring Boot 4 双分支上线

2026 年最重磅的升级：框架完成 **Spring Boot 4** 全量迁移，并新增 `6.x` 与 `6.x-single` 两个分支，分支矩阵扩展为 **5 个变体**（master / single / workflow / 6.x / 6.x-single），业务代码、四层架构与开发规范保持完全一致。

#### 技术栈基线升级
- **Spring Boot 4.1.0**：核心框架升级到 Spring Boot 4 基线，JDK 21
- **Jackson 3 迁移**：`ruoyi-common-json`、`ruoyi-common-redis`、`ruoyi-common-sensitive`、`ruoyi-common-serialMap` 全量迁移到 Jackson 3
- **Redisson 4**：分布式缓存组件同步升级
- **Starter 适配**：Sa-Token、MyBatis-Plus、Dynamic-Datasource 等 starter 完成 Spring Boot 4 适配
- **AutoConfiguration 重定位**：适配 SB4 自动装配机制，批量验证 33 个通用模块零改动通过

#### 容器与安全回归
- **Web 容器切换**：`server.undertow` → `server.jetty`，适配 Spring Boot 4 容器体系
- **不安全 HTTP 方法禁用**：弃用 Undertow 后补回禁用 TRACE/TRACK 等不安全方法的功能
- **全项目编译收官**：主代码 + 测试代码 test-compile 全绿，含 monitor-admin、snailjob-server 扩展模块

### 7.2 全站国际化 (i18n)

2026 年 1 月完成前端**全站国际化**改造，实现真正的多语言开箱即用。

#### 组件与页面国际化
- **全组件覆盖**：AModal、ADetail、ATable、AForm 系列、AFilePreview、AImportExcel、AAiChat、图表卡片等全部组件接入国际化
- **系统页面国际化**：用户、角色、部门、字典、公告、API 密钥、在线设备、代码生成器、工作流、商城等页面全面多语言
- **路由标题国际化**：路由元数据 `title` 支持国际化 key，菜单与导航栏动态多语言
- **字典国际化增强**：字典标签与字典数据支持多语言渲染
- **语言切换性能优化**：优化语言包加载与切换逻辑，切换无感知
- **代码生成器适配**：生成模板默认输出国际化代码

### 7.3 异常监控与错误日志系统

新增完整的**后端异常自动监控**能力，问题可追溯、可治理。

- **异步记录**：通过 `@Async` 异步记录所有后端异常，不阻塞主业务流程
- **智能去重**：基于 Redis 实现 1 分钟窗口去重，相同错误（异常类型 + URI + 消息）只记一条，重复自动累加 `occurrence_count`
- **全上下文采集**：记录异常堆栈、请求信息（URI / 方法 / 参数 / IP）、用户信息（ID / 姓名 / 部门 / 租户）、服务器环境（主机名 / IP / 版本）
- **双场景 Token 解析**：同时支持 OpenAPI 请求与普通请求的用户信息解析
- **归属监控模块**：错误日志从 system 迁移到 monitor 模块，权限标识 `monitor:errorLog`，新增 `request_pattern` 字段与客户端版本记录
- **日志降噪**：4xx 客户端异常日志由 error 降级为 warn

### 7.4 多租户能力增强

- **数据源级排除表**：支持在数据源级别配置租户排除表，更细粒度的租户隔离控制
- **会话级动态切换**：实现租户动态切换的会话级别隔离，切换后自动清理，避免串租户
- **Pool↔Silo 双向迁移**：支持共享库（Pool）与独立库（Silo）之间的双向租户搬迁（对标 Azure 多租户范本）
- **迁移加固**：分批搬运 + 迁移停写窗口，保障大数据量迁移一致性
- **集团库幂等迁移器**：`GroupSchemaMigrator` 实现版本幂等的集团库结构迁移

### 7.5 AI 深度思考升级

LangChain4j 完成 **0.35.0 → 1.14.1** 大版本升级，全面支持大模型**深度思考（Thinking）**。

- **大版本升级**：LangChain4j 0.35.0 → 1.14.1，通义千问依赖迁移为 `langchain4j-community-dashscope:1.14.0-beta24`
- **深度思考支持**：新增 `enableThinking`、`thinkingBudgetTokens`、`reasoningEffort`、`returnThinking` 配置项
- **五大模型全适配**：DeepSeek、Anthropic Claude、通义千问 Qwen、Ollama、OpenAI 五个 provider 全部注入推理参数
- **API 迁移**：`ChatLanguageModel`/`StreamingChatLanguageModel` → `ChatModel`/`StreamingChatModel`，`generate()` → `chat()`，新增 `onPartialThinking` 流式思考回调
- **WebSocket 协议增强**：流式响应增加 `phase`（思考 / 正文阶段）+ `reasoningContent`（思考过程），前端可实时展示模型思考链
- **Token 统计适配**：`TokenCounter` 适配 1.x 的 `TokenCountEstimator`

### 7.6 支付能力扩展

- **支付宝当面付**：新增支付宝当面付接入，支持扫码支付与付款码支付
- **支付兜底与幂等**：条码支付撤销兜底 + 支付成功幂等处理，避免重复入账
- **微信支付 V3 公钥模式**：补全 `publicKeyId` 字段，区别于证书序列号，留空自动回退兼容
- **配置集群同步**：平台配置与支付配置新增集群节点间同步机制，多实例配置一致

### 7.7 移动端全面进化

#### UniApp X 与原生渲染
- **UniApp X 迁移**：`plus-app` 迁移到 UniApp X，缓存工具重构为类实现并适配 UTS
- **nvue 原生渲染**：开启 nvue 原生渲染支持，关键页面获得原生级性能
- **图片压缩 Composable**：新增跨端 `useImageCompress` 图片压缩组合式函数，`plus-uniapp` 与 `plus-app` 同步支持

#### WD UI 组件库增强
- **新增组件**：SlideVerify 滑动验证、Avatar 头像组件
- **开放能力**：wd-button 新增 `getRealtimePhoneNumber` 实时手机号获取能力
- **ConfigProvider 重构**：配置提供者重构，主题与全局配置更灵活
- **15+ 组件增强**：calendar、cell、cell-group、curtain、drop-menu、input、keyboard、loading、picker-view、rate、search、segmented、status-tip、swiper、table、tabs、textarea 等组件能力增强

#### 行业 Demo 模板重写
- **20+ 行业模板全面重写**：涵盖商城（基础 / 生鲜）、生活服务（外卖配送 / 医疗健康）、社交（博客 / 论坛）、资讯（基础新闻 / 科技新闻）、教育（在线考试 / 在线学堂）、金融（预算管理）、游戏（益智 / 短视频）、企业官网、工具（计算器 / 天气）等场景，开箱即用

### 7.8 前端组件与体验升级

- **通用文件预览**：新增 AFilePreview 通用文件预览组件，支持全屏预览
- **富文本增强**：AFormEditor 支持 Markdown 粘贴解析
- **地图选点升级**：AFormMap 搜索框接入 POI 候选下拉，输入即出列表、选中即定位
- **表格选择增强**：AFormTableSelect 支持图片类型与 `initialLabel` 编辑回显
- **本地 SVG 图标**：Icon 组件新增本地 SVG sprite 图标源（基于 `vite-plugin-svg-icons`），登录页品牌 logo 切换为真·SVG
- **记住密码加密**：记住密码功能新增本地存储加密
- **双列菜单布局**：新增双列菜单布局模式
- **交互细节**：AStatsCard 数字滚动动画、404 页面倒计时自动跳转、表格选中行高亮
- **大数据量性能优化**：菜单管理、部门 / 目录树改用 `el-table-v2` 虚拟滚动 + 搜索防抖，解决展开全部与大数据搜索卡死

### 7.9 后端架构与功能优化

- **返回类型规范**：Service / DAO 层方法返回类型由 `boolean` 调整为 `int`，返回受影响行数，语义更明确
- **更新包装器增强**：`PlusLambdaUpdate` 增强，支持跨数据库兼容的模糊匹配
- **统一消息推送**：新增通知消息系统，支持短信、邮件统一推送
- **社交登录模块**：新增第三方社交登录功能模块
- **客户端授权校验**：新增客户端授权模块完整性校验
- **OSS 能力增强**：根据 URL 获取 OSS 文件信息的缓存功能，富文本入库前清洗私有桶预签名参数
- **开发日志优化**：开发环境日志文件输出 + SQL 蓝色高亮，提升本地排查效率

### 7.10 智能开发体系扩展

框架内置的 Claude Code 智能开发体系持续进化，形成完整的**自主开发闭环**。

- **自主开发流水线**：整合 `/kickoff` + `/dev-loop` + `/loop-gen` + `project-init`，对接自主循环开发（autonomous-dev），实现「方案 → 计划 → 执行」原生闭环
- **经验沉淀机制**：新增 `/exp` 经验沉淀命令与 exp-sediment 技能，开发经验自动积累复用
- **业务模块裁剪**：新增 module-strip 模块裁剪技能（`/strip-modules`），内置微信生态等多个预设，一键裁掉不需要的业务模块
- **技能体系扩容**：技能与命令持续扩展（新增 e2e-test、writing-plans、log-audit、env-config、project-migration、framework-sync 等）
- **凭据安全**：git-workflow 远程操作优先走 Sigil 凭据保险库，明文 token 不落地

---

**2026年更新总结**：

2026 年上半年，框架在**技术栈现代化**、**企业级能力**和**智能化开发**三条主线上全面演进：

- <Icon icon="lucide:rocket" /> **Spring Boot 4**：全量迁移 SB4 + Jackson 3 + Redisson 4，分支矩阵扩展为 5 个变体
- <Icon icon="lucide:earth" /> **全站国际化**：前端组件、页面、路由、字典全面多语言，开箱即用
- <Icon icon="lucide:shield" /> **异常可观测**：异步 + 去重的错误日志系统，问题可追溯可治理
- <Icon icon="lucide:building-2" /> **多租户进阶**：Pool↔Silo 双向迁移、会话级动态切换、集团库幂等迁移
- <Icon icon="lucide:bot" /> **AI 深度思考**：LangChain4j 1.14.1，五大模型深度思考 + 思考链实时推送
- <Icon icon="lucide:credit-card" /> **支付扩展**：支付宝当面付、微信 V3 公钥模式、配置集群同步
- <Icon icon="lucide:smartphone" /> **移动端进化**：UniApp X + nvue 原生渲染、WD 新组件、20+ 行业模板
- <Icon icon="lucide:brain" /> **自主开发**：Claude Code 自主开发流水线 + 经验沉淀 + 模块裁剪

---

每个端的重构都遵循模块化、标准化的原则，确保代码质量和可维护性。框架从架构分层、AI 能力集成、物联网通信、页面设计器一路演进，到 2026 年已完成 **Spring Boot 4 现代化升级**、**全站国际化**、**AI 深度思考**、**多租户 Pool↔Silo 迁移**与**移动端 UniApp X 进化**，并形成 master / single / workflow / 6.x / 6.x-single 五分支矩阵，为企业级应用提供更强大的技术支撑和更高效的全栈开发体验。
